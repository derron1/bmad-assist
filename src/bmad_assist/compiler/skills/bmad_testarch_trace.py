...
from bmad_assist.compiler.workflows.testarch_trace import TestarchTraceCompiler
...
class BmadTestarchTraceCompiler(SkillLayoutCompilerBase):
    """:class:`WorkflowCompiler` for the v6.4+ ``bmad-testarch-trace`` skill."""

    skill_id = SKILL_ID
    legacy_workflow_name = "testarch-trace"
    legacy_compiler_class = TestarchTraceCompiler
    ...
    def build_extra_vars(
        ...
        """No workflow-specific extras for testarch-trace.

        The legacy compiler resolves ``{epic_num}`` / ``{test_dir}`` /
        ``{source_dir}`` / ``{gate_type}`` / ``{date}`` downstream
        during its own ``compile()`` call (gate_type defaults to
        ``"epic"``). The SKILL.md body references runtime tokens
        (``{user_name}``, ``{communication_language}``,
        ``{workflow.*}`` blocks, ``{skill-root}`` / ``{skill-name}`` /
        ``{project-root}``) that the variable engine resolves before
        the LLM transform layer.
        """
        return {}
    ...
    def validate_context(self, context: CompilerContext) -> None:
        ...
        The legacy ``TestarchTraceCompiler`` (via the tri-modal base)
        checks ``epic_num`` and the workflow directory's existence.
        ``story_num`` is optional — trace operates over an entire epic
        (or release / hotfix gate) and reads ALL stories in the epic
        ...
        if not skill_md.is_file():  # pragma: no cover — _locate_skill_md raises first
            raise CompilerError(
                ...
                f"  Why it's needed: Source for the bmad-testarch-trace skill\n"
