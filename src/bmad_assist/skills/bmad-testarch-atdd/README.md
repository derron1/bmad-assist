# bmad-testarch-atdd (bundled BMAD v6.4 TEA skill source)

This directory holds the bundled copy of the upstream `bmad-testarch-atdd`
...
`.claude/skills/bmad-testarch-atdd/` (or
`.agents/skills/bmad-testarch-atdd/`) when one exists; this bundled
...
- `checklist.md` — ATDD validation checklist, referenced from
  `SKILL.md`.
- `atdd-checklist-template.md` — output template for the ATDD
  checklist artifact.
- `steps-c/`, `steps-v/`, `steps-e/` — tri-modal micro-step files
  (Create / Validate / Edit) loaded just-in-time during workflow
  execution.
- `resources/knowledge/*.md` — TEA knowledge fragments (fixtures,
  selectors, patterns) referenced by the step files.
...
(`SKILL.md`, `customize.toml`, `checklist.md`, `*-template.md`,
`steps-c/*.md`, `steps-v/*.md`, `steps-e/*.md`,
`resources/knowledge/*.md`, `resources/tea-index.csv`) verbatim from
the new `.claude/skills/bmad-testarch-atdd/` mirror. Do not hand-edit
them here; project-specific overrides belong in
`_bmad/custom/bmad-testarch-atdd.toml` (team) or
`bmad-testarch-atdd.user.toml` (personal).
