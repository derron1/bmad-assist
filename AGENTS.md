...
**Sub-step prompt manipulation IS what bmad-assist exists to do.** Upstream BMAD skills are interactive and human-driven — they `<ask>` for input, write to files, and expect a person in the loop. bmad-assist adapts them for headless multi-LLM orchestration via two complementary layers, and uses both itself:
1. **Regex patches** in [compiler/patching/](src/bmad_assist/compiler/patching/) operate on the *compiled* SKILL.md+steps body (post-substitution). They strip `<ask>` blocks, insert output markers (`<!-- VALIDATION_REPORT_START -->`), embed file paths, remove interactive `<check>`/`<invoke-task>` calls, and rewrite step bodies. Two sub-layers — see "Patch layers" under Key concepts. Application point: [compiler/skills/_base.py:476-490](src/bmad_assist/compiler/skills/_base.py#L476-L490).
2. **`customize.toml` deep-merge** ([skill_layout/resolver.py](src/bmad_assist/skill_layout/resolver.py)) is the BMAD-native customization surface — facts, activation hooks, `on_complete`. Limited to the keys the SKILL.md author exposed; cannot rewrite step bodies.

Patches and customize.toml are not alternatives. The experiment harness exposes both as AB axes (`patch_set`, `customize_set`) for the same reason bmad-assist uses both internally: they vary different surfaces.
...
- **Customization layers** (merged in order):
  1. Bundled `customize.toml` in `src/bmad_assist/skills/<skill>/` — framework default, top of file says "DO NOT EDIT — auto-updated only while unmodified"
  2. Project `_bmad/custom/<skill>.toml` — project-level override
  3. Project `_bmad/custom/<skill>.user.toml` — user-level override
- **Customization surface**: NEVER edit installed skill `customize.toml` at runtime unless you intend a local override. Bootstrap records the bundled hash in `.bundle-version`: if the installed file still matches the prior bundled hash, it is auto-updated; if it diverged, it is preserved and a warning is logged. Project overrides should go in `{project-root}/_bmad/custom/`. The bundled file in `src/bmad_assist/skills/` IS source-of-truth for the framework default; edit there only when knowingly changing the default.
- **Patch layers** (orthogonal to customization layers — both alive, distinct purposes):
  1. **Universal defaults** — [src/bmad_assist/default_patches/defaults.yaml](src/bmad_assist/default_patches/defaults.yaml) and [defaults-testarch.yaml](src/bmad_assist/default_patches/defaults-testarch.yaml). Shipped via `pyproject.toml` package-data. Applied to **every** compile (testarch defaults only fire for `bmad-testarch-*`). Home for universal cleanup rules — `<ask>` stripping, sprint-status removal, headless-confirmation gate stripping, etc.
  2. **Per-workflow opt-in overrides** — `<consumer-project>/.bmad-assist/patches/<workflow>.patch.yaml`. Discovered at runtime by [compiler/patching/discovery.py](src/bmad_assist/compiler/patching/discovery.py) (search order: project → global → package-fallback). Optional; consumer projects ship empty by default. The 15 files at the bmad-assist repo's own `.bmad-assist/patches/` are (a) reference fixtures for the e2e suite (`tests/skill_layout/test_*_e2e.py` `shutil.copy2`s them into tmp projects), (b) the active project layer when running bmad-assist from its own repo, (c) documented in skill READMEs as the recommended consumer override location. Phase 7 (`0781990`, 2026-04-29) deleted per-workflow patches from the SHIPPED package-fallback path; the runtime discovery layer and dev-dir copies survived intentionally.
...
- **Bootstrap re-syncs bundled skills on every run.** Per-project `customize.toml` at `.claude/skills/<skill>/customize.toml` / `.agents/skills/<skill>/customize.toml` is hash-gated: unchanged bundled defaults are overwritten with the new bundled default, while files whose hash differs from the prior bundled default are preserved as local overrides. The SOURCE bundled `customize.toml` in `src/bmad_assist/skills/` is the framework default.
...
- **`.bmad-assist/patches/<workflow>.patch.yaml` files are NOT dead bytes.** Easy to misread: Phase 7 deleted per-workflow patches from `src/bmad_assist/default_patches/` (the SHIPPED package-fallback path), so consumer pip installs no longer carry them. But the runtime discovery layer at [compiler/patching/discovery.py](src/bmad_assist/compiler/patching/discovery.py) still probes the project's own `.bmad-assist/patches/` dir, and the 15 files at the bmad-assist repo's own `.bmad-assist/patches/` are live: e2e test fixtures (`shutil.copy2`'d into tmp projects), the project-local layer when running bmad-assist from its own repo, and documented in skill READMEs as the consumer override location. Don't conflate "removed from shipped fallback" with "mechanism is obsolete." See "Patch layers" under Key concepts.
- **Universal defaults must run even when no per-workflow patch exists.** Historical bug at [compiler/skills/_base.py:480-483](src/bmad_assist/compiler/skills/_base.py#L480-L483) and [compiler/shared_utils.py](src/bmad_assist/compiler/shared_utils.py) `apply_post_process` short-circuited when `patch_path is None`, silently skipping bundled `defaults*.yaml` for any consumer project without an opt-in patch. The dead `else` branch in `_apply_patch_post_process` (with comment about anchoring at the package directory) was the giveaway. Defaults must always merge in; per-workflow patches add on top. If you find post-process being skipped on a no-patch path, that's a regression.
...
- Don't edit `src/bmad_assist/skills/<skill>/customize.toml` thinking it is "just a config" — it is the framework default and lands in every consumer project whose installed copy has not been locally modified.
...
- Don't delete files in `.bmad-assist/patches/` thinking they're dev cruft. They're live e2e fixtures + the project-layer for running bmad-assist from its own repo (and `tests/skill_layout/test_*_e2e.py` `shutil.copy2`s them by absolute path — deletion breaks ~15 tests at collection). Phase 7 only retired the SHIPPED package-fallback copies under `src/bmad_assist/default_patches/`, not the runtime discovery mechanism.
- Don't add inline regex stripping logic to compiler modules (`compiler/skills/*.py`) when the patches mechanism would do the job. New universal cleanup rules go in `default_patches/defaults.yaml` (or `defaults-testarch.yaml` for TEA-only). New per-workflow rules go in a `.patch.yaml` file. The patches surface is what the experiment harness (`patch_set` AB axis) and consumer overrides target — bypassing it makes the rule invisible to both.
