"""Side-by-side comparison: legacy vs skill-layout testarch-nfr compile.

Note: legacy un-prefixed name is ``testarch-nfr-assess`` (the v6.4+
canonical drops the ``-assess`` suffix). Both names route to the same
compiler.
"""
...
LEGACY_WORKFLOW = REPO_ROOT / "src" / "bmad_assist" / "workflows" / "testarch-nfr-assess"
...
def _install_new_layout(project_root: Path) -> None:
    """Mirror the bundled v6.4+ skill at the canonical bmad-testarch-nfr path."""
...
def _install_old_layout(project_root: Path) -> Path:
    """Map ``testarch-nfr-assess`` → ``nfr-assess`` per WORKFLOW_TO_BMAD_DIR."""
    target = project_root / "_bmad" / "bmm" / "workflows" / "testarch" / "nfr-assess"
    target.parent.mkdir(parents=True, exist_ok=True)
...
class TestSideBySideCompatibility:
    ...
    def test_both_paths_produce_compiled_workflows(self, dual_project: Path) -> None:
        ...
        old = compile_workflow(
            "testarch-nfr-assess", _make_context(dual_project), skill_layout="old"
        )
        new = compile_workflow(
            "bmad-testarch-nfr", _make_context(dual_project), skill_layout="new"
        )
...
# Diff thresholds — nfr-assess's legacy instructions.md is ~36 lines,
# but the v6.4+ NFR skill ships substantially more step files
# (security/performance/reliability/scalability subagents) than the
# other "small" TEA workflows, and the embedded TEA knowledge expands
# the new-side body. Empirical numbers run ~1075–1086 lines; cap at
# 1500 (~30% headroom). If this later overshoots, raise it WITH
# RATIONALE rather than tightening to mask regressions.
...
def test_legacy_vs_new_compile_diff_is_bounded(
    ...
    old = compile_workflow("testarch-nfr-assess", _make_context(dual_project), skill_layout="old")
    new = compile_workflow(
        "bmad-testarch-nfr", _make_context(dual_project), skill_layout="new"
    )
    diff = list(
        difflib.unified_diff(
            ...
            fromfile="legacy/testarch-nfr-assess",
            tofile="new/bmad-testarch-nfr",
            lineterm="",
            n=0,
        )
    )
    ...
    assert len(change_lines) <= threshold, (
        f"diff between legacy and new testarch-nfr exceeds {threshold} lines "
