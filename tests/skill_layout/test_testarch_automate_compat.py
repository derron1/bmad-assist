"""Side-by-side comparison: legacy vs skill-layout testarch-automate compile.

Mirror of :mod:`tests.skill_layout.test_testarch_atdd_compat`. Stands
up a single project that satisfies *both* compiler paths and asserts
a bounded set of structural invariants.
"""
...
LEGACY_WORKFLOW = REPO_ROOT / "src" / "bmad_assist" / "workflows" / "testarch-automate"
...
def _install_old_layout(project_root: Path) -> Path:
    """Map ``testarch-automate`` → ``automate`` per WORKFLOW_TO_BMAD_DIR."""
    target = project_root / "_bmad" / "bmm" / "workflows" / "testarch" / "automate"
    target.parent.mkdir(parents=True, exist_ok=True)
...
class TestSideBySideCompatibility:
    ...
    def test_both_paths_produce_compiled_workflows(self, dual_project: Path) -> None:
        ...
        old = compile_workflow("testarch-automate", _make_context(dual_project), skill_layout="old")
        new = compile_workflow(
            "bmad-testarch-automate", _make_context(dual_project), skill_layout="new"
        )
...
# Diff thresholds — automate's legacy instructions.md is ~43 lines (one
# of the smaller TEA workflows), so the diff is dominated by the new
# SKILL.md (~85 lines) and the embedded TEA knowledge fragments
# being added on the new side. Empirical numbers run ~750–770 lines
# regex-only; mocked-LLM body is shorter so provider_available=True
# tracks closer to ~370. We cap at 1000 (~30% headroom) on both
# axes.
...
def test_legacy_vs_new_compile_diff_is_bounded(
    ...
    diff = list(
        difflib.unified_diff(
            ...
            fromfile="legacy/testarch-automate",
            tofile="new/bmad-testarch-automate",
            lineterm="",
            n=0,
        )
    )
    ...
    assert len(change_lines) <= threshold, (
        f"diff between legacy and new testarch-automate exceeds {threshold} lines "
