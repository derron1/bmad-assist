"""Side-by-side comparison: legacy vs skill-layout testarch-trace compile.

trace's legacy ``instructions.md`` is ~1047 lines (the largest of the
TEA workflows), so we use a larger diff cap. See atdd's compat
threshold rationale for the same pattern.
"""
...
LEGACY_WORKFLOW = REPO_ROOT / "src" / "bmad_assist" / "workflows" / "testarch-trace"
...
def _install_old_layout(project_root: Path) -> Path:
    ...
    target = project_root / "_bmad" / "bmm" / "workflows" / "testarch" / "trace"
    target.parent.mkdir(parents=True, exist_ok=True)
...
class TestSideBySideCompatibility:
    ...
    def test_both_paths_produce_compiled_workflows(self, dual_project: Path) -> None:
        ...
        old = compile_workflow("testarch-trace", _make_context(dual_project), skill_layout="old")
        new = compile_workflow(
            "bmad-testarch-trace", _make_context(dual_project), skill_layout="new"
        )
...
# Diff thresholds — trace's legacy instructions.md is ~1047 lines (the
# largest TEA workflow). The diff is dominated by legacy lines deleted
# from the new-side body. Empirical numbers run high; cap at 1800
# (~30% headroom over the observed ~1400 range).
_DIFF_THRESHOLD_PROVIDER_TRUE = 1800
_DIFF_THRESHOLD_PROVIDER_FALSE = 1800
...
def test_legacy_vs_new_compile_diff_is_bounded(
    ...
    diff = list(
        difflib.unified_diff(
            ...
            fromfile="legacy/testarch-trace",
            tofile="new/bmad-testarch-trace",
            lineterm="",
            n=0,
        )
    )
    ...
    assert len(change_lines) <= threshold, (
        f"diff between legacy and new testarch-trace exceeds {threshold} lines "
