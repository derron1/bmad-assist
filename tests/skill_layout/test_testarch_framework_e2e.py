"""End-to-end tests for the Phase 3.2-B ``bmad-testarch-framework`` compiler.
...
PATCH_FILE = REPO_ROOT / ".bmad-assist" / "patches" / "testarch-framework.patch.yaml"
...
def _make_context(project_root: Path) -> CompilerContext:
    ...
    return CompilerContext(
        ...
        # framework is project-level; story_num optional.
...
class TestSkillLayoutCompileE2E:
    ...
    def test_compile_returns_well_shaped_compiled_workflow(self, project_root: Path) -> None:
        ...
        result = compile_workflow(
            "bmad-testarch-framework", _make_context(project_root), skill_layout="new"
        )
        assert result.workflow_name == "bmad-testarch-framework"
        body = result.context
        assert body
        assert "<compiled-workflow>" in body
        assert "Test Framework Setup" in body or "framework" in body.lower()
    ...
    def test_skill_layout_new_routes_through_new_path(self, project_root: Path) -> None:
        ...
        compiler = get_workflow_compiler(
            "bmad-testarch-framework", skill_layout="new", project_root=project_root
        )
        assert type(compiler).__module__.startswith("bmad_assist.compiler.skills.")
        assert type(compiler).__name__ == "BmadTestarchFrameworkCompiler"
    ...
    def test_legacy_workflow_name_routes_to_new_compiler_when_layout_new(
        ...
        compiler = get_workflow_compiler(
            "testarch-framework", skill_layout="new", project_root=project_root
        )
        assert type(compiler).__module__.startswith("bmad_assist.compiler.skills.")
        assert type(compiler).__name__ == "BmadTestarchFrameworkCompiler"
...
class TestSkillLayoutCacheLifecycle:
    ...
    def _cache_paths(self, project_root: Path) -> tuple[Path, Path]:
        ...
        return (
            cache_dir / "bmad-testarch-framework.tpl.xml",
            cache_dir / "bmad-testarch-framework.tpl.xml.meta.yaml",
        )
    ...
    def test_cache_invalidates_when_customize_toml_changes(self, project_root: Path) -> None:
        ...
        customize = (
            project_root / ".claude" / "skills" / "bmad-testarch-framework" / "customize.toml"
        )
    ...
    def test_cache_meta_records_skill_layout_mode_and_hashes(self, project_root: Path) -> None:
        ...
        assert meta["skill_id"] == "bmad-testarch-framework"
...
class TestSkillLayoutLLMTransforms:
    ...
    def test_compile_with_provider_invokes_apply_llm_transforms(
        ...
        def fake_apply(*, content, transforms, provider_config, config, phase_name, workflow_label, **_):
            ...
            # Patch must_contain: /[Ff]ramework|[Cc]onfig/ + /[Tt]est/.
            # Note: the framework patch's post_process collapses
            # "Playwright Test" → "playwright", so we cannot rely on
            # the "Test" inside that phrase to satisfy /[Tt]est/.
            # Inject a separate "test" token in the config blurb.
            transformed = (
                "<workflow>\n"
        ...
        assert captured
        assert captured["workflow_label"] == "bmad-testarch-framework"
        assert captured["phase_name"] == "testarch_framework"
        assert isinstance(captured["transforms"], list)
        assert len(captured["transforms"]) >= 1
        assert "Test Framework Setup" in captured["content"]
