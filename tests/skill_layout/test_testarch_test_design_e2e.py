"""End-to-end tests for the Phase 3.2-B ``bmad-testarch-test-design`` compiler.
...
PATCH_FILE = REPO_ROOT / ".bmad-assist" / "patches" / "testarch-test-design.patch.yaml"
...
def _make_context(project_root: Path) -> CompilerContext:
    ...
    return CompilerContext(
        ...
        # test-design is epic- or system-level; story_num optional.
...
class TestSkillLayoutCompileE2E:
    ...
    def test_compile_returns_well_shaped_compiled_workflow(self, project_root: Path) -> None:
        ...
        assert result.workflow_name == "bmad-testarch-test-design"
        body = result.context
        assert body
        assert "<compiled-workflow>" in body
        assert "Test Design" in body or "test design" in body.lower()
    ...
    def test_skill_layout_new_routes_through_new_path(self, project_root: Path) -> None:
        ...
        compiler = get_workflow_compiler(
            "bmad-testarch-test-design", skill_layout="new", project_root=project_root
        )
        assert type(compiler).__module__.startswith("bmad_assist.compiler.skills.")
        assert type(compiler).__name__ == "BmadTestarchTestDesignCompiler"
    ...
    def test_legacy_workflow_name_routes_to_new_compiler_when_layout_new(
        ...
        compiler = get_workflow_compiler(
            "testarch-test-design", skill_layout="new", project_root=project_root
        )
        assert type(compiler).__module__.startswith("bmad_assist.compiler.skills.")
        assert type(compiler).__name__ == "BmadTestarchTestDesignCompiler"
...
class TestSkillLayoutCacheLifecycle:
    ...
    def _cache_paths(self, project_root: Path) -> tuple[Path, Path]:
        ...
        return (
            cache_dir / "bmad-testarch-test-design.tpl.xml",
            cache_dir / "bmad-testarch-test-design.tpl.xml.meta.yaml",
        )
    ...
    def test_cache_invalidates_when_customize_toml_changes(self, project_root: Path) -> None:
        ...
        customize = (
            project_root / ".claude" / "skills" / "bmad-testarch-test-design" / "customize.toml"
        )
    ...
    def test_cache_meta_records_skill_layout_mode_and_hashes(self, project_root: Path) -> None:
        ...
        assert meta["skill_id"] == "bmad-testarch-test-design"
...
class TestSkillLayoutLLMTransforms:
    ...
    def test_compile_with_provider_invokes_apply_llm_transforms(
        ...
        from bmad_assist.compiler.skills import bmad_testarch_test_design as skill_mod
        ...
        def fake_apply(*, content, transforms, provider_config, config, phase_name, workflow_label, **_):
            ...
            # Patch must_contain: /[Dd]esign/ + /[Tt]est/.
            transformed = (
                "<workflow>\n"
                '<step n="1" goal="Design">'
                "<action>Produce an epic-level test design plan.</action>"
                "</step>\n"
                "</workflow>"
            )
            return transformed, [
        ...
        assert captured
        assert captured["workflow_label"] == "bmad-testarch-test-design"
        assert captured["phase_name"] == "testarch_test_design"
        assert isinstance(captured["transforms"], list)
        assert len(captured["transforms"]) >= 1
        assert "Test Design" in captured["content"]
