"""Side-by-side comparison: legacy vs skill-layout testarch-test-design compile."""
...
LEGACY_WORKFLOW = REPO_ROOT / "src" / "bmad_assist" / "workflows" / "testarch-test-design"
...
def _install_old_layout(project_root: Path) -> Path:
    ...
    target = project_root / "_bmad" / "bmm" / "workflows" / "testarch" / "test-design"
    target.parent.mkdir(parents=True, exist_ok=True)
...
class TestSideBySideCompatibility:
    ...
    def test_both_paths_produce_compiled_workflows(self, dual_project: Path) -> None:
        ...
        old = compile_workflow(
            "testarch-test-design", _make_context(dual_project), skill_layout="old"
        )
        new = compile_workflow(
            "bmad-testarch-test-design", _make_context(dual_project), skill_layout="new"
        )
...
# Diff thresholds — test-design's legacy instructions.md is ~52 lines.
# See automate's compat for the rationale; same shape applies.
...
def test_legacy_vs_new_compile_diff_is_bounded(
    ...
    diff = list(
        difflib.unified_diff(
            ...
            fromfile="legacy/testarch-test-design",
            tofile="new/bmad-testarch-test-design",
            lineterm="",
            n=0,
        )
    )
    ...
    assert len(change_lines) <= threshold, (
        f"diff between legacy and new testarch-test-design exceeds {threshold} lines "
