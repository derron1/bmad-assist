"""End-to-end tests for the Phase 3.2-B ``bmad-testarch-trace`` compiler.
...
BUNDLED_SKILL = REPO_ROOT / "src" / "bmad_assist" / "skills" / "bmad-testarch-trace"
PATCH_FILE = REPO_ROOT / ".bmad-assist" / "patches" / "testarch-trace.patch.yaml"


def _install_skill(project_root: Path) -> Path:
    """Mirror the bundled skill into the project's ``.claude/skills/`` directory."""
    target = project_root / ".claude" / "skills" / "bmad-testarch-trace"
...
def _make_context(project_root: Path) -> CompilerContext:
    ...
    return CompilerContext(
        ...
        # trace operates over an entire epic; story_num optional.
...
class TestSkillLayoutCompileE2E:
    ...
    def test_compile_returns_well_shaped_compiled_workflow(self, project_root: Path) -> None:
        ...
        result = compile_workflow(
            "bmad-testarch-trace", _make_context(project_root), skill_layout="new"
        )
        assert result.workflow_name == "bmad-testarch-trace"
        body = result.context
        assert body
        assert "<compiled-workflow>" in body
        assert "Coverage Traceability" in body or "trace" in body.lower()
    ...
    def test_skill_layout_new_routes_through_new_path(self, project_root: Path) -> None:
        ...
        compiler = get_workflow_compiler(
            "bmad-testarch-trace", skill_layout="new", project_root=project_root
        )
        assert type(compiler).__module__.startswith("bmad_assist.compiler.skills.")
        assert type(compiler).__name__ == "BmadTestarchTraceCompiler"
    ...
    def test_legacy_workflow_name_routes_to_new_compiler_when_layout_new(
        ...
        compiler = get_workflow_compiler(
            "testarch-trace", skill_layout="new", project_root=project_root
        )
        assert type(compiler).__module__.startswith("bmad_assist.compiler.skills.")
        assert type(compiler).__name__ == "BmadTestarchTraceCompiler"
...
class TestSkillLayoutCacheLifecycle:
    ...
    def _cache_paths(self, project_root: Path) -> tuple[Path, Path]:
        ...
        return (
            cache_dir / "bmad-testarch-trace.tpl.xml",
            cache_dir / "bmad-testarch-trace.tpl.xml.meta.yaml",
        )
    ...
    def test_cache_is_written_on_first_compile(self, project_root: Path) -> None:
        ...
        compile_workflow("bmad-testarch-trace", _make_context(project_root), skill_layout="new")
    ...
    def test_cache_invalidates_when_customize_toml_changes(self, project_root: Path) -> None:
        ...
        customize = project_root / ".claude" / "skills" / "bmad-testarch-trace" / "customize.toml"
    ...
    def test_cache_meta_records_skill_layout_mode_and_hashes(self, project_root: Path) -> None:
        ...
        assert meta["skill_id"] == "bmad-testarch-trace"
...
class TestSkillLayoutLLMTransforms:
    ...
    def test_compile_with_provider_invokes_apply_llm_transforms(
        ...
        from bmad_assist.compiler.skills import bmad_testarch_trace as skill_mod
        ...
        def fake_apply(*, content, transforms, provider_config, config, phase_name, workflow_label, **_):
            ...
            # Patch must_contain:
            #   /[Tt]race|[Mm]atrix|[Cc]overage/ + /[Rr]equirement/.
            transformed = (
                "<workflow>\n"
                '<step n="1" goal="Trace">'
                "<action>Generate a coverage matrix mapping each requirement to tests.</action>"
                "</step>\n"
                "</workflow>"
            )
            return transformed, [
        ...
        compile_workflow("bmad-testarch-trace", _make_context(project_root), skill_layout="new")
        assert captured
        assert captured["workflow_label"] == "bmad-testarch-trace"
        assert captured["phase_name"] == "testarch_trace"
        assert isinstance(captured["transforms"], list)
        assert len(captured["transforms"]) >= 1
        assert "Coverage Traceability" in captured["content"]
