"""Side-by-side comparison: legacy vs skill-layout testarch-ci compile.

Mirror of :mod:`tests.skill_layout.test_testarch_atdd_compat`.
"""
...
LEGACY_WORKFLOW = REPO_ROOT / "src" / "bmad_assist" / "workflows" / "testarch-ci"
...
def _install_old_layout(project_root: Path) -> Path:
    ...
    target = project_root / "_bmad" / "bmm" / "workflows" / "testarch" / "ci"
    target.parent.mkdir(parents=True, exist_ok=True)
...
class TestSideBySideCompatibility:
    ...
    def test_both_paths_produce_compiled_workflows(self, dual_project: Path) -> None:
        ...
        old = compile_workflow("testarch-ci", _make_context(dual_project), skill_layout="old")
        new = compile_workflow("bmad-testarch-ci", _make_context(dual_project), skill_layout="new")
...
# Diff thresholds — ci's legacy instructions.md is ~38 lines (small).
# New SKILL.md adds ~85 lines and TEA knowledge fragments expand the
# embedded context. We cap at 1000 to mirror automate's empirical
# range with headroom.
...
def test_legacy_vs_new_compile_diff_is_bounded(
    ...
    diff = list(
        difflib.unified_diff(
            ...
            fromfile="legacy/testarch-ci",
            tofile="new/bmad-testarch-ci",
            lineterm="",
            n=0,
        )
    )
    ...
    assert len(change_lines) <= threshold, (
        f"diff between legacy and new testarch-ci exceeds {threshold} lines "
