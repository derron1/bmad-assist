"""Side-by-side comparison: legacy vs skill-layout testarch-test-review compile.

test-review's legacy ``instructions.md`` is ~628 lines, so we use a
larger diff cap than the smaller TEA workflows. See atdd's compat
...
LEGACY_WORKFLOW = REPO_ROOT / "src" / "bmad_assist" / "workflows" / "testarch-test-review"
...
def _install_old_layout(project_root: Path) -> Path:
    ...
    target = project_root / "_bmad" / "bmm" / "workflows" / "testarch" / "test-review"
    target.parent.mkdir(parents=True, exist_ok=True)
...
class TestSideBySideCompatibility:
    ...
    def test_both_paths_produce_compiled_workflows(self, dual_project: Path) -> None:
        ...
        old = compile_workflow(
            "testarch-test-review", _make_context(dual_project), skill_layout="old"
        )
        new = compile_workflow(
            "bmad-testarch-test-review", _make_context(dual_project), skill_layout="new"
        )
    ...
    def test_both_embed_story_file(self, dual_project: Path) -> None:
        ...
        for body, label in [(old.context, "legacy"), (new.context, "new")]:
            assert "10-1-foo" in body, f"{label} body should reference the story by key"


# Diff thresholds — test-review's legacy instructions.md is ~628 lines
# (large), so the diff is dominated by legacy lines deleted from the
# new-side body. Empirical numbers run high; cap at 1500 (~30%
# headroom over the observed ~1100–1200 range).
...
def test_legacy_vs_new_compile_diff_is_bounded(
    ...
    diff = list(
        difflib.unified_diff(
            ...
            fromfile="legacy/testarch-test-review",
            tofile="new/bmad-testarch-test-review",
            lineterm="",
            n=0,
        )
    )
    ...
    assert len(change_lines) <= threshold, (
        f"diff between legacy and new testarch-test-review exceeds {threshold} lines "
